...
class _BeeamvoHomeState extends State<BeeamvoHome>
  ...
  /// User-facing copy when the recorder produced no usable audio bytes.
  static String _noAudioCapturedMessage() {
    if (Platform.isMacOS) {
      return 'No audio was captured.\n'
          '1) System Settings → Privacy & Security → Microphone → enable Beeamvo\n'
          '2) Settings → General → Audio Input Device → System Default\n'
          '3) Hold the hotkey a bit longer (toggle mode is more reliable for tests)';
    }
    return 'No audio was captured. Check the microphone in '
        'Settings → General → Audio Input Device.';
  }
