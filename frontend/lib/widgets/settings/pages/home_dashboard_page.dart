...
class _HomeDashboardPageState extends State<HomeDashboardPage> {
  ...
  Widget _buildActivitySection(BuildContext context) {
    final recentDays = widget.statsService.getRecentDays(30);
    final maxVal = recentDays.fold<int>(0, (a, b) => math.max(a, b));
    final activeDays = recentDays.where((v) => v > 0).length;
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            const Expanded(child: BeeGroupLabel(label: 'Last 30 Days')),
            Text(
        ...
        Container(
          ...
          child: Wrap(
            spacing: 4,
            runSpacing: 4,
            children: List.generate(recentDays.length, (i) {
              final val = recentDays[i];
              final intensity = maxVal == 0 ? 0.0 : val / maxVal;
              return Tooltip(
                message: _recentDayTooltip(i, val),
                child: _ActivityDot(
                  intensity: intensity,
                  color: beeYellow(context),
                  emptyColor: beeBorder(context),
                ),
              );
            }),
          ),
...
// ═════════════════════════════════════════════════════════════════════════════
// Activity Dot — single cell in the 30-day heatmap
// ═════════════════════════════════════════════════════════════════════════════
...
class _ActivityDot extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        shape: BoxShape.circle,
        color: color.withValues(alpha: 0.15 + (intensity * 0.85)),
      ),
    );
  }
}
