...
class HotkeyConfig {
  ...
  ///
  /// [defaultTo] controls which [HotkeyConfig] is returned when the stored
  /// JSON is missing, malformed, or references an unknown key. The legacy
  /// behavior for every caller was to fall back to [defaultHotkey]
  /// (Ctrl+Shift+V), which silently rebound the clipboard-popup and
  /// mode-selection actions onto the main dictate hotkey. Pass the
  /// action-appropriate default so a corrupt value can never collide.
  factory HotkeyConfig.fromJson(
    String jsonStr, {
    HotkeyConfig defaultTo = defaultHotkey,
  }) {
    try {
