...
static NSString * ggml_metal_load_source_with_includes(NSString * path_source, NSError ** error) {
    NSString * src = [NSString stringWithContentsOfFile:path_source encoding:NSUTF8StringEncoding error:error];
    if (src == nil || (error && *error != nil)) {
        return nil;
    }

    NSString * source_dir = [path_source stringByDeletingLastPathComponent];
    NSArray<NSString *> * header_names = @[ @"ggml-common.h", @"ggml-metal-impl.h" ];

    for (NSString * header_name in header_names) {
        NSString * header_path = [source_dir stringByAppendingPathComponent:header_name];
        NSString * header_src = [NSString stringWithContentsOfFile:header_path encoding:NSUTF8StringEncoding error:error];
        if (header_src == nil || (error && *error != nil)) {
            return nil;
        }

        NSString * include_stmt = [NSString stringWithFormat:@"#include \"%@\"", header_name];
        src = [src stringByReplacingOccurrencesOfString:include_stmt withString:header_src];

        NSString * embed_marker = [NSString stringWithFormat:@"__embed_%@__", header_name];
        src = [src stringByReplacingOccurrencesOfString:embed_marker withString:header_src];
    }
...
ggml_metal_library_t ggml_metal_library_init(ggml_metal_device_t dev) {
    ...
    // TODO: move to a function
    {
        ...
        if (path_lib != nil) {
            ...
            src = ggml_metal_load_source_with_includes(path_source, &error);
            if (error) {
                GGML_LOG_ERROR("%s: error: %s\n", __func__, [[error description] UTF8String]);
                return nil;
            }
